...
def select_models(choices):
    ...
    for index in choices:
        if index == 0:
            # LDA with simplified hyperparameters
            param_grid = {
            ...
            # KNN with specified range of neighbors
            param_grid = {
                'estimator__n_neighbors': [ 5, 7, 31]
            }
            model = GridSearchCV(OneVsRestClassifier(KNeighborsClassifier()), param_grid, cv=5, scoring='accuracy')
            name = "KNN"
        elif index == 2:
            # SVM with simplified hyperparameters
            param_grid = {
...
def train_sklearn_models(choices, train, valid, measures, path):
    ...
    for name, model in tqdm(all_models.items(), desc="Training Models"):
        ...
        plot_confusion_matrix(valid.labels, valid_preds, class_names, f"{path}/{name}_confusion_matrix.png")
